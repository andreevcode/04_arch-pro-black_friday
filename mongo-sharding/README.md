# mongo-sharding
## Как запустить?
1. Запускаем кластер mongodb (сервер конфигурации, 2 шарда, роутер) и приложение
```shell
docker compose up --build -d
```
2. Выполняем скрипт инициализации для сервера конфигурации и шардов. 
```shell
...
- Выполнить скрипт [./scripts/mongo_check-shards.sh](scripts/mongo_check-shards.sh) и проверить кол-во документов на каждом шарде.
