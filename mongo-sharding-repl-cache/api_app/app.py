...
from fastapi import Body, FastAPI, HTTPException, status, Request, Response
...
async def custom_users_key_builder(
        func,
        namespace: str = "",
...
@cache(
    expire=60,
    key_builder=custom_users_key_builder
)
# @cache(expire=60 * 1)
...
async def invalidate_users_cache():
    if REDIS_URL:
        ...
        await redis_client.delete(f"users_cache")
